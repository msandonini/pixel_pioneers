...
from fusion.encoder import FusionEncoder
...
def evaluate(
    ...
    for batch in loader:
        ...
        fused_ref = fusion_enc(ref_emb)
        fused_dist = fusion_enc(dist_emb)
...
def main():
    ...
    model_save_path = Path(conf["weights_out"]) / "fusion_enc" / dt / "model.pt"
    model_save_path_latest = Path(conf["weights_out"]) / "fusion_enc" / "model_latest.pt"
    ...
    model_save_dict = {
        "fusion_state_dict": fusion_enc.state_dict(),
